...
%%% calls the q learning function, then finds -ve log likelihood for each
%%% parameter, normalises (per S-A transition, to fit on same scale) then
%%% plots. (all for different numbers of trials)

environment='graph5';

qpass=main_q_learner_function(environment,10, 0.1);
probarrays(:,:,1)=old_parameterplot([0.1 8 0.8 0]);
normplots10(:,:)=probarrays(:,:,1)/10;

qpass=main_q_learner_function(environment,50, 0.1);
probarrays(:,:,2)=old_parameterplot([0.1 8 0.8 0]);
normplots50(:,:)=probarrays(:,:,2)/149;

qpass=main_q_learner_function(environment,100, 0.1);
probarrays(:,:,3)=old_parameterplot([0.1 8 0.8 0]);
normplots100(:,:)=probarrays(:,:,3)/160;

qpass=main_q_learner_function(environment,500, 0.1);
probarrays(:,:,4)=old_parameterplot([0.1 8 0.8 0]);
normplots500(:,:)=probarrays(:,:,4)/283;
...
% plot(x,normplots5000(:,1),'Color',[0 1 0],'LineWidth',1)
 ...
 ylabel('Normalised negative log likelihood')
  legend('10 trials','50 trials','100 trials','500 trials','1000 trials', 'alpha')
...
% plot(x,normplots5000(:,2),'Color',[0 1 0],'LineWidth',1)
 ...
 ylabel('Normalised negative log likelihood')
  legend('10 trials','50 trials','100 trials','500 trials','1000 trials', 'beta')
...
% plot(x,normplots5000(:,3),'Color',[0 1 0],'LineWidth',1)
 ...
 ylabel('Normalised negative log likelihood')
  legend('10 trials','50 trials','100 trials','500 trials','1000 trials', 'gamma')
...
% plot(x,normplots5000(:,4),'Color',[0 1 0],'LineWidth',1)
 ...
 ylabel('Normalised negative log likelihood')
 legend('10 trials','50 trials','100 trials','500 trials','1000 trials','Q(0)')
